...
<script language="javascript">
        	function add(){
                var url="<%=path %>/wjyw/ycPaper_add.jsp";
        		top.Dialog.open({URL:url,Title:"外检纸张信息添加",Width:600,Height:400});
           	}
            ...
            function update(id){    
                	 var url = "<%=path %>/<%=preActoin %>!preUpdate.action?id="+id;
                     top.Dialog.open({URL:url,Title:"外检纸张信息修改"});
...
<div id="searchPanel" class="box2" panelTitle="外检纸张配置查询" roller="false">
	<form action="<%=preActoin %>!query.action" method="post" name="queryForm">
	<input name="index" type="hidden" value="${page.index }" />
	<table>
		<tr>
			<td>纸张代码：</td>
			<td><input name="ycPaper.takecode" type="text" value="${ycPaper.takecode}"/></td>
			<td>纸张名称：</td>
			<td><input name="ycPaper.takeName" type="text" value="${ycPaper.takeName}"/></td>
...
<div id="scrollContent" class="table_bor">
	<table class="tableStyle" mode="list" useCheckbox="true" selectRowButtonOnly="false">
		<tr>
			...
			<th><span>纸张类别</span></th>
			<th><span>纸张说明</span></th>
			<th><span>备注</span></th>
		</tr>
		<c:forEach var="entity" items="${list_data}" varStatus="status">
			<tr >
				...
				<td align="center">&nbsp;${entity.takeName}</td>
				<td align="center">&nbsp;${entity.takeType}</td>
