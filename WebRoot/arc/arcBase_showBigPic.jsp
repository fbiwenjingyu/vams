	...
	<div class="bigPic">
		...
		<div class="info">
			<table>
			  <tbody>
			  <tr>
				 <c:forEach 	var="pic" items="${pic_list}" varStatus="status">
					<td align="center"><input type="hidden" value="${pic.id}" id="cur${status.index}"/>
	 				<div align="center"><img align="center" id="ys${status.index}" <c:if test="${pic.paperId!='164'}"> height="818" width="818" </c:if> border="0" onmousewheel="return bigimg(this)"  src='arcBaseInfo!getBase64ToImage.action?tplj=${pic.tplj}'  /></div></td>
	    		</c:forEach>
			</tr>
			  </tbody>
			</table>
		...
		$(".bigPic a.btnleft").click(function(){
			...
			if(curid<0){
				...
				for(var i=0;i<$ifr.length;i++){//循环取出页面上的iframe
					...
					ifr.find('#arcpic'+curPicpath).removeClass("out");		
				}
			}
			$("#curid").val(curid);
		})
