...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <script type="text/javascript">
    	...
    	function showMessage(){
    		var ids=checkOne();
			if(null != ids){
	    		var url="swgl/stoouts-showOutArc.action?applyIds="+ids;
  ...
  <body>
<!-- 标题栏、查询栏 -->
<div id="searchPanel" class="box2" panelTitle="出库档案  | 查询" roller="false">
<form action="stoouts-outlist.action" method="post" name="queryForm">
	<input name="index" type="hidden" value="${page.index}" /><!-- 当前页input -->
	<input id="fuzzy" name="fuzzy" type="hidden" value="${fuzzy}"/> 
	<table id="selector">
		<tr>
			<td>批次号：</td>
			<td><input name="stoout.ckpch" type="text" style="width:126px" value="${stoout.ckpch}"/></td>
			<td>系统编号：</td>
			<td><input name="stoout.xtdabh" type="text" style="width:126px" value="${stoout.xtdabh}"/></td>
			<td>申请人：</td>
			<td><input name="stoout.sqrid" type="text" style="width:126px" value="${stoout.sqrid}"/></td>
			<td>档案编号：</td>
			<td><input name="stoout.dabh" type="text" style="width:126px" value="${stoout.dabh}"/></td>
			<td>业务类型：</td>
		...
		<tr>
			<td>储位编号：</td>
			<td><input name="stoout.cwbh" type="text" style="width:126px" value="${stoout.cwbh}"/></td>
			<td>流水号：</td>
			<td><input name="stoout.lsh" type="text" style="width:126px" value="${stoout.lsh}"/></td>
			<td>号牌种类：</td>
			<td><select prompt="请选择" name="stoout.hpzl" style="width:126px"  data='${vs:hpzlSelect()}' selectedValue="${stoout.hpzl}" ></select></td>
			<td>号牌号码：</td>
			<td><input name="stoout.hphm" type="text" value="${stoout.hphm}"/></td>
			<td>车辆识别代号</td>
			<td><input name="stoout.clsbdh" type="text" style="width:126px" value="${stoout.clsbdh}"/></td>
...
<div id="scrollContent" class="table_bor">
	<table class="tableStyle" mode="list" useCheckbox="true" selectRowButtonOnly="false">
		<tr>
			...
			<th><span>批次号</span></th>
			<th><span>出库申请人</span></th>
			<th><span>申请时间</span></th>
			<th><span>出库审核人</span></th>
			<th><span>出库时间</span></th>
			<th><span>档案状态</span></th>
		</tr>
	<c:forEach var="info" items="${pageList}" varStatus="status">
			<td class="ali02"><input type="radio" name="indexnum" value="${info.xtdabh}" pch="${info.ckpch}"/></td>
			...
			<td class="ali02"><fmt:formatDate value="${info.cksj}" pattern="yyyy年MM月dd日 HH:mm:ss"/></td>
