...
//		freshTop("swgl/checkin-query1.action");
	...
	//档案迁移
	function moveInfo(btn){
		var cwbh=btn.name;
		var cwbh1;
		if(cwbh.length!=8){
			cwbh1=cwbh.substring(2).substring(0,5);
			cwbh=cwbh.substring(2);
		}else{
			cwbh1=cwbh.substring(0,5);
		}
		var url="swda/storeGet-moveTable.action?cwbh="+cwbh1+"&ids="+cwbh;
		var diag = new top.Dialog();
			diag.Title = cwbh1+"档案迁移";
			diag.URL = url;
			diag.Width=1000;
			diag.Height=550;
	...
	//刷新迁移过的档案
	function flsuhInfo(oldcwbh,newcwbh){
		newcwbh=newcwbh.split(";")[0];
		$("#"+oldcwbh).html(newcwbh).attr("id",newcwbh);
	...
	<div id="dmain" style="overflow: auto; " >
		...
		<tbody>
			...
			<tr class="ali02" daid="${info.xtdabh}">
				...
				<td class="ali02">
					<button name="${info.xtdabh}" type="button" onclick="printInfo('${info.xtdabh}',this)"><span class="icon_code">打签</span></button>
				</td>
			</tr>
			</c:forEach>
		</tbody>
