<%@ page language="java" pageEncoding="UTF-8"%>  <!-- 公共基础引包入头文件 -->
<!--框架必需start-->
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/js/jquery.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/js/language/cn.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/js/framework.js"></script>
<link href="${pageContext.request.contextPath}/libs/css/import_basic.css" rel="stylesheet" type="text/css"/>
<link href="${pageContext.request.contextPath}/libs/skins/blue/style.css" rel="stylesheet" type="text/css" id="theme" themeColor="blue" positionTarget="positionContent"/>
<link rel="stylesheet" type="text/css" id="skin" prePath="${pageContext.request.contextPath}/"/>
<link rel="stylesheet" type="text/css" id="customSkin"/>
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/js/nav/spliter.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/js/layout_main.js"></script>
<!--框架必需end-->
<!--引入弹窗组件start-->
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/js/popup/drag.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/js/popup/dialog.js"></script>
<!--引入弹窗组件end-->
<!-- 表单验证控件 -->
<script src="${pageContext.request.contextPath}/libs/js/form/validationRule.js" type="text/javascript"></script>
<script src="${pageContext.request.contextPath}/libs/js/form/validation.js" type="text/javascript"></script>
<!-- 日期控件 -->
<script src="${pageContext.request.contextPath}/libs/js/form/datePicker/WdatePicker.js" type="text/javascript" ></script>
<!-- 基本选项卡 -->
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/js/nav/basicTabModern.js"></script>

<!--修正IE6不支持PNG图start-->
<!--<style>
img {
	behavior:url("${pageContext.request.contextPath}/libs/js/method/pngFix/pngbehavior.htc");
}
</style>
--><!--修正IE6不支持PNG图end-->
 

 
<!-- 自定义库 start-->
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/jmap.js" type="text/javascript"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/vams-commons.js" type="text/javascript"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/vams-utils.js" type="text/javascript"></script>
<!-- 自定义css -->
<link href="${pageContext.request.contextPath}/libs/page.css" rel="stylesheet" type="text/css"/>
<script type="text/javascript" src="${pageContext.request.contextPath}/libs/page.js"></script>
<!-- 自定义库end -->