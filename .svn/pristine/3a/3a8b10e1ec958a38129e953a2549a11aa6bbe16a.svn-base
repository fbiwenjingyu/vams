...
public class ArcBaseInfoService extends BaseService{
	...
	public String getScanTypeByDeptCode(String deptCode) {
		Dept dept = deptDAO.selectByPrimaryKey(deptCode);
		return dept.getF2();
	}
	 ...
	 * @return
	 */
	public List<ArcBaseInfo> getArcBaseInfoByHphmAndYwlx(String hphm,
			String ywlx) {
		ArcBaseInfoExample example = new ArcBaseInfoExample();
		example.createCriteria().andHphmEqualTo(hphm).andYwlxEqualTo(ywlx);
		return arcBaseInfoDAO.selectByExample(example);
	}
