  ...
  <head>
	...
	<script type="text/javascript">
		...
		function smt(){
			...
			//获取提交数据
			var shiId=$("input[name='shi_id']").val().trim();//室id
			var guiIds="";//柜ids
...
/**  信息填写验证end  *** */			
			...
			//请求数据
			var reqData={"shiId":shiId,"guiIds":guiIds,"lieNum":lieNum,"geNum":geNum,"fenNum":fenNum,"cflx":cflx,"remark":remark};
			//提交
			$.post("cwgl/stoset-addShi.action",reqData,function(data){
		    	alert(data[1]);
				if(data[0]=="1"){
  ...
  <div class="box1"  style="height: 530px;overflow: auto;">
    	<table id="fTable" border="1" align="center" class="tableStyle" formMode="line">
    		<tr>
    			<td>档案室编号（1位数字）：</td><td><input name="shi_id" /></td>
    		</tr>
    		<tr>
    		...
    		<tfoot>
    			<tr>
    				<td colspan="2"><button onclick="smt()">确定</button><button onclick="closeWin()">取消</button></td>
    			</tr>
    		</tfoot>
    	</table>
   </div>
