...
<!-- 标题栏、查询栏 -->
<div id="searchPanel" class="box2" panelTitle="档案综合查询" roller="false">
<form action="arcQuery-list.action" method="post" name="queryForm">
	...
	<table id="selector">
		<tr>
			...
			<td>号牌种类：</td>
			<td><select name="info.hpzl"  prompt="请选择"  style="width:126px" selectedValue="${info.hpzl}" data='${vs:hpzlSelect()}'></td>
			<td>号牌号码：</td>
		...
		<tr>
			...
			<td>业务类型：</td>
			<td><select name="info.ywlx" prompt="请选择" selectedValue="${info.ywlx}" data='${vs:ywlxSelect()}'></td>
			<td>创建人账号：</td>
			<td><input name="info.cjr" type="text" style="width:126px" value="${info.cjr}"/></td>
			<td>档案状态：</td>
			<td><select name="info.dazt"  prompt="请选择"  style="width:126px" selectedValue="${info.dazt}" data='${vs:daztSelect()}'></td>
...
<div id="scrollContent" class="table_bor">
	...
	<c:forEach var="info" items="${pageList}" varStatus="status">
		<tr align="center" ondblclick="imgInfo('查看扫描页')">
