...
<html>
  <head>
    ...
    <script type="text/javascript">
		...
		function checkin(){
			...
			if($ck.length==0){
				...
				var diag = new top.Dialog();
					diag.Title = "批次核对入库";
					diag.URL = "swgl/stoins-checkList.action?pch="+pch;
		...
		function stoin(){
			var ids=checkMany();
			if(null != ids){
				top.Dialog.confirm("确定入库？",function(){
					stoin_opera("2",ids);
				});
			}
		}
    	...
    	function stoin_opera(type,ids){
			$.post("stoins-checkin.action",{"type":type,"applyIds":ids},function(data){
				if(data[0]=="1"){
					alert(data[1]);
					//刷新当前页
					freshFrom();
		...
		function backin_opera(type,ids){
			top.Dialog.confirm("确定退回？",function(){
				$.post("stoins-backin.action",{"type":type,"applyIds":ids},function(data){
					if(data[0]=="1"){
						alert(data[1]);
						//刷新当前页
						freshFrom();
  ...
  <body>
<!-- 标题栏、查询栏 -->
<div id="searchPanel" class="box2" panelTitle="核对入库  | 查询" roller="false">
<form action="stoins-toCheckin.action" method="post" name="queryForm">
	<input name="index" type="hidden" value="${page.index}" /><!-- 当前页input -->
	<input id="fuzzy" name="fuzzy" type="hidden" value="${fuzzy}"/> 
	<table id="selector">
		<tr>
			<td>批次号：</td>
			<td><input name="stoin.rkpch" type="text" value="${stoin.rkpch}"/></td>
			<td>系统编号：</td>
			<td><input name="stoin.xtdabh" type="text" value="${stoin.xtdabh}"/></td>
			<td>申请人：</td>
			<td><input name="stoin.sqrid" type="text" value="${stoin.sqrid}"/></td>
			<td>档案编号：</td>
			<td><input name="stoin.dabh" type="text" value="${stoin.dabh}"/></td>
			<td>业务类型：</td>
		...
		<tr>
			<td>储位编号：</td>
			<td><input name="stoin.cwbh" type="text" value="${stoin.cwbh}"/></td>
			<td>流水号：</td>
			<td><input name="stoin.lsh" type="text" value="${stoin.lsh}"/></td>
			<td>号牌种类：</td>
			<td><select prompt="请选择" name="stoin.hpzl"  data='${vs:hpzlSelect()}' selectedValue="${stoin.hpzl}" ></select></td>
			<td>号牌号码：</td>
			<td><input name="stoin.hphm" type="text" value="${stoin.hphm}"/></td>
			<td>车辆识别代号</td>
			<td><input name="stoin.clsbdh" type="text" value="${stoin.clsbdh}"/></td>
...
<div id="operation" class="box_tool_min">
	...
	<div class="right">
		...
		<a href="javascript:void(0);" onclick="checkin()"><span class="icon_layers" >批次核对入库</span></a>
		<div class="box_tool_line"></div>
		<a href="javascript:void(0);" onclick="stoin()"><span class="icon_mark">入库</span></a>
		<div class="box_tool_line"></div>
...
<div id="scrollContent" class="table_bor">
	...
	<c:forEach var="info" items="${pageList}" varStatus="status">
			<td align="center"><input type="checkbox" name="indexnum" value="${info.xtdabh}" pch="${info.rkpch}"/></td>
			<td align="center">${status.index+1}</td>
			<td align="center">${info.rkpch}</td>
			...
			<td align="center">${vs:userName(info.sqrid)}（${info.sqrid}）</td>
