...
<div id="searchPanel" class="box2" panelTitle="条码查询" roller="false">
	<form action="<%=preActoin %>!ycInfo_codeQuery.action" method="post" name="queryForm">
	...
	<table>
		<tr>		
			...
			<td>是否外检：</td>
			<td><select  id="status" name="ycinfo.sfwj" >
					<option value="" >全部</option>
					<option value="Y" <c:if test="${ycinfo.sfwj=='Y'}">selected</c:if>>有外检</option>
					<option value="N" <c:if test="${ycinfo.sfwj=='N'}">selected</c:if>>无外检</option>
				</select></td>
