...
//    		$div.render();
    	...
    	*/
    	function ajaxData($inp,type,arc){
    		//查询档案
    		$.post("stoouts-findArc.action",{"applyIds":arc,"type":type},function(data){
    			unsignAll();
				if(data[0]=="1"){
		    		...
		    		showOk("档案："+arc+" 可出库，数量："+data[1].length+"份。");//消息提示
				}else{
    	...
    	function showMessage(div){
    		var url="swgl/stoouts-showArc.action?applyIds="+div.xtdabh;
...
//			var apname=$("input[name='applyName']").val();
			...
			$.post("stoouts-applyOut.action",{"applyIds":apsids},function(data){
			    //alert(data[0]);
				//签收完成
				if(data[0]=="1"){
					printData=data[1];
    	...
    	function printSign(){
    		...
    		var url="stoouts-printCkd.action?pch="+printData.pch+"&sl="+printData.sl+"&name="+printData.sqrid+"&time="+printData.time;
    		window.open(url,'打印出库签收单','width=400,height=450');
    	}
    	
    	
    	/**继续出库*/
    	function storagein(){
    		freshTop("stoouts-form.action");
    	}
...
<div>
 	...
 	<div>
	     <table id="apply_msg" class="tableStyle">
		 	<tr>
		 		<th colspan="2"><span style="font-size: 20px;font-weight: bold;">档案预出库</span></th>
		 	</tr>
			<tr>
			<!-- 
				<td width="10%" class="ali03">出库申请人：</td><td width="10%"><input name="applyName" type="text" class="validate[required,length[1,10]]"/><span class="star">*</span></td>
			-->
