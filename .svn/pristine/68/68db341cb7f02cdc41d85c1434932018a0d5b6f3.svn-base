			  ...
			  <tr>
				 <c:forEach 	var="pic" items="${ycPic_list}" varStatus="status">
					<td align="center"><input type="hidden" value="${pic.id}" id="cur${status.index}"/>
	 				<div align="center"><img align="center" id="ys${status.index}" height="818" border="0" onmousewheel="return bigimg(this)" width="818" src='arcBaseInfo!getNotBase64ToImagePath2.action?picPath=${pic.picpath}'  /></div></td>
	    		</c:forEach>
			</tr>
			  </tbody>
			</table>
		...
		$(".bigPic a.btnleft").click(function(){
			...
			if(curid<0){
				...
				for(var i=0;i<$ifr.length;i++){//循环取出页面上的iframe
					...
					ifr.find('#ycpic'+curPicpath).removeClass("out");		
				}
			}
			$("#curid").val(curid);
		})
